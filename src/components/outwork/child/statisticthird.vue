  ...
  <div class="container" style="height: 750px;overflow-y: scroll">
    ...
    <el-row class="search">
          ...
          <el-col :span="2" class="top-schear-but">
              <el-button type="warning" style="width: 100%;" size="small" @click="outExcel()">
                导出Excel
              </el-button>
          </el-col>
          <el-col :span="2" class="top-schear-but">
    ...
    <el-row>
      ...
      <el-col :span="3">
        <span>体检次数：{{totalStatistic.physicalCount || 0}}</span>
      </el-col>
      <el-col :span="3">
        <span>入校：{{totalStatistic.enrolledCount || 0}}</span>
      </el-col>
      <el-col :span="3">
        <span>入园：{{totalStatistic.entergardenCount || 0}}</span>
      </el-col>
      <el-col :span="2">
          入托: {{totalStatistic.nurseryCount || 0}}
      </el-col>
      <el-col :span="3" :offset="5">
          到访人数: {{totalStatistic.visitCount || 0}}
      </el-col>
      <el-col :span="3">
          消费金额: {{totalStatistic.totalMoney || 0}}
      </el-col>
    </el-row>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async outExcel() {
      ...
      // exportMethod("体检人员统计",envUrl,exportArg,this)
    ...
    async getCities(item, type) {
      ...
      if (item) {
        ...
        try {
          return new Promise((resolve, reject) => {
            getCity({proviceId : proviceVal,userId:this.userId}).then(res => {
              resolve(res);
